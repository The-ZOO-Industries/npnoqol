...
namespace mapping
{
	...
	static auto load_class_mappings(const nlohmann::json& jclass, const char** class_target, const char** signature) noexcept
		-> void
	{
		...
		case mapping_mode::MCP:
			target = jclass.value("mcp", "");
			break;
		case mapping_mode::OBF:
			target = jclass.value("obf", "");
			break;
		...
		if (target.empty())
		{
			std::println("[ERROR] Missing class mapping for current mode");
			return;
		}
	...
	static auto load_attributes(const nlohmann::json& attributes, std::map<std::string, const char**> mapping) noexcept
		-> void
	{
		for (auto& [key, value] : attributes.items())
		{
			if (mapping.contains(key))
			{
				...
				{
					std::println("[ERROR] Missing attribute mapping for '{}'", key);
				}
			}
		}
	}
	...
	namespace minecraft
	{
		...
		inline const char* runTick{ nullptr };
	}
		...
		-> bool
	{
		...
		try
		{
			auto jsonObject = nlohmann::json::parse(mapping::json, nullptr, false);
			if (jsonObject.is_discarded())
			{
				std::println("[ERROR] Failed to parse mapping JSON");
				return false;
			}

			const auto classesIt{ jsonObject.find("classes") != jsonObject.end()
				? jsonObject.find("classes")
				: jsonObject.find("Classes") };

			if (classesIt == jsonObject.end() || !classesIt->is_object())
			{
				std::println("[ERROR] Mapping JSON is missing the classes object");
				return false;
			}

			const auto& classesArray = *classesIt;
		...
		{
			std::println("[ERROR] Mapping init failed: {}", exception.what());
			return false;
		}
	}
}
